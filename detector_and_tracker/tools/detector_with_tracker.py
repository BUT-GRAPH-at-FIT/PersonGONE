...
def make_parser():
    ...
    parser.add_argument("--roi_expand", default=0.1, type=float, help="ROI expansion in percentsof are (0-1)")
    parser.add_argument("--save_video", action="store_true", help="whether to save the inference result of video")
    #################################################
    parser.add_argument("--use_crop", action="store_true", help="whether to save the inference result of video")
    #################################################
    parser.add_argument("--tracker", type=str, default='SORT', help="which tracker to use (SORT/BYTE)")
    # exp file
    parser.add_argument(
        "-f",
        "--exp_file",
...
class Predictor(object):
    def __init__(
        ...
        if args.tracker == 'SORT':
            ...
            # self.tracker = Sort(max_age=20, min_hits=3, iou_threshold=0.05)
        elif args.tracker == 'BYTE':
...
#########################################################################################################
        if args.tracker == 'SORT':
            ...
            cls = output[:, 6]
            scores = np.expand_dims(output[:, 4] * output[:, 5], -1)
            ...
            detections, classes, class_confs, det_confs = self.tracker.update(dets, cls.tolist(), class_confs.tolist(), det_confs.tolist())
            dets = [{'det' : d[:4],
                     'track_id' : int(d[4]),
                     'class' : classes[pos][-1],
                     'class_conf' : class_confs[pos][-1],
                     'det_conf' : det_confs[pos][-1]}
                      for pos, d in enumerate(detections)]
        else:
            frame_results = None
            if output is not None:
                online_targets = self.tracker.update(output, ratio)
                online_tlwhs = []
                online_ids = []
                online_scores = []
                online_scores_objects = []
                online_scores_classes = []
                online_class_ids = []
                for t in online_targets:
                    tlwh = t.tlwh
                    vertical = tlwh[2] / tlwh[3] > 1.6
                    if tlwh[2] * tlwh[3] > self.min_box_area and not vertical:
                        online_tlwhs.append(t.tlbr)
                        online_ids.append(t.track_id)
                        online_scores.append(t.score)
                        online_scores_objects.append(t.score_object)
                        online_scores_classes.append(t.score_class)
                        online_class_ids.append(t.class_id)
                # save results
                frame_results = {'dets' : online_tlwhs,
                                 'track_ids' : online_ids,
                                 'scores' : online_scores,
                                 'score_objects' : online_scores_objects,
                                 'score_classes' : online_scores_classes,
                                 'class_ids' : online_class_ids}
            dets = [{
                    'det' : frame_results['dets'][pos],
                    'track_id' : frame_results['track_ids'][pos],
                    'class' : frame_results['class_ids'][pos],
                    'class_conf' : frame_results['score_classes'][pos],
                    'det_conf' : frame_results['score_objects'][pos]
                    }
                    for pos in range(len(frame_results['dets']))]
...
#########################################################################################################
    ...
    def apply_roi(self, img):
        if self.roi is not None:
            if self.use_crop:
                return img[self.roi[1]:self.roi[3], self.roi[0]:self.roi[2], :]
            blur_bckg = np.full(img.shape, (255,255,255), dtype=np.uint8)
            # blur_bckg = np.full(img.shape, (0,0,0), dtype=np.uint8)
...
def proceede_video(predictor, vis_folder, current_time, args):
    ...
    if args.use_crop:
        width = predictor.roi[2]-predictor.roi[0]
        height = predictor.roi[3]-predictor.roi[1]
    # save_name = '{:s}_{:d}_{:s}_{:d}'.format(os.path.basename(args.path).split('.')[0], args.tsize, args.tracker, args.track_buffer)
    ...
    while True:
        ...
        if ret_val:
            ...
            frame = predictor.apply_roi(frame)
...
def main(exp, args):
    ...
    assert args.tracker in ('SORT, BYTE')
